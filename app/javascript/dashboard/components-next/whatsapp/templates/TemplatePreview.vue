  ...
  <div class="flex flex-col gap-3">
    ...
    <div class="flex flex-col items-end gap-1">
      ...
      <div
        ...
        class="flex flex-wrap w-full max-w-sm gap-1.5"
      >
        <div
          v-for="(btn, idx) in buttons"
          :key="idx"
          class="flex items-center gap-1.5 rounded-lg bg-n-solid-2 px-3 py-1.5 text-xs font-medium text-n-blue-11 outline outline-n-container"
        >
          <Icon :icon="buttonIcon(btn.type)" class="size-3" />
          <span>{{ btn.text || '...' }}</span>
        </div>
      </div>
    </div>
