class Api::V1::Accounts::ContactsController < Api::V1::Accounts::BaseController
  ...
  before_action :set_items_per_page, only: [:index, :active, :search, :filter]
  ...
  def set_items_per_page
    @items_per_page = params[:per_page] || RESULTS_PER_PAGE
  end
  ...
  def fetch_contacts_with_has_more(contacts)
    ...
    results = filtrate(contacts)
              .includes(includes_hash)
              .offset(offset)
              .limit(@items_per_page.to_i + 1)
              .to_a

    @has_more = results.size > @items_per_page.to_i
    results = results.first(@items_per_page.to_i) if @has_more
